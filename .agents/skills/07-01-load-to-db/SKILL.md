---
name: 07-01-load-to-db
description: 06-02-review-translation完了後のCSVを検証しDB（Postgres互換）へ保存する。LLMの判断を含まない決定論的な実行のみ。パイプラインの最終ステップとして使う。
---

# Skill: DBへの格納

## 目的
`06-02-review-translation`完了後の`<論文名>.csv`を検証し、DB（Postgres互換）へ保存する。本Skill自体はLLMによる判断を含まない、決定論的な実行のみ。
...
## エラー時の対応
- 検証エラーが出た場合、該当する`sentence_id`を`<論文名>.csv`で確認し、原因のSkill（`06-01-translate`/`06-02-review-translation`等）側の課題として報告する
...
完了条件を満たしていれば確認を挟まず次のSkillへ進む（本Skillはパイプライン最終ステップのため実際には次のSkillは無い）。満たしていない場合は本Skill内の該当手順（またはエラー時の対応）からやり直す。完了条件は満たしているが判断に迷う点・懸念がある場合のみ、その旨を添えて人間またはCodex/Claude Codeに相談する。
