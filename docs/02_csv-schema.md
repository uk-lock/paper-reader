...
`05-01-split-sentences`〜`07-01-load-to-db`の間、`output/<論文名>/`配下のCSVが前処理パイプラインの
...
[05-01-split-sentences](../.claude/skills/05-01-split-sentences/SKILL.md)（`postprocess/src/split_sentences.py`）が生成し、
`05-02-review-split`・`06-01-translate`・`06-02-review-translation`が値を埋めながら`07-01-load-to-db`まで引き継がれる、1論文＝1ファイルのCSV。
...
| `type` | text | `heading` / `body` / `caption` / `footnote` / `reference` / `image` のいずれか（下表参照） |
...
| `translated_text` | text (空欄可) | 翻訳文（日本語）。`06-01-translate`〜`06-02-review-translation`で埋める。`type=reference`/`type=image`は翻訳対象外のため常に空欄 |
...
| `image` | 図・表の画像参照（Markdown画像記法。`03-01-upload-images`がDrive URLへ書き換え済み） | ×（`translated_text`は空欄のまま） |

### 書き込みルール

- **`05-01-split-sentences`完了以降、`original_text`・`order`・`sentence_id`・`type`・`heading_level`列は変更しない。**
  変更対象は`05-02-review-split`での分割誤り修正、および`translated_text`列のみ
...
- `07-01-load-to-db`（`load_to_db.py`）は上記の列構成・`type`値・翻訳要否を保存前に検証する
  （不正があれば保存せずエラー終了。詳細は[03_db.md](03_db.md#dbへの格納処理load_to_dbpy)）

## `<論文名>_flags.csv`（分割レビュー用の自動検出候補）

`split_sentences.py`が`<論文名>.csv`と同時に出力する。`05-02-review-split`が消費し、パイプライン上のファイルとしては残らない。
...
`postprocess/src/check_translation.py`が`06-01-translate`完了後のCSVに対して出力する。`06-02-review-translation`が消費する。
...
| `image_should_not_be_translated` | `type=image`なのに`translated_text`が設定されている（対象外行への誤翻訳） |

## 関連ドキュメント

- [01_workflow.md](01_workflow.md): これらのCSVが生成・更新される前処理パイプライン全体の流れ
- [03_db.md](03_db.md): `07-01-load-to-db`によるDBへの保存先スキーマ
