...
完了条件を満たしていれば確認を挟まず次のSkillへ進む（本Skillはパイプライン最終ステップのため実際には次のSkillは無い）。満たしていない場合は本Skill内の該当手順（またはエラー時の対応）からやり直す。完了条件は満たしているが判断に迷う点・懸念がある場合のみ、その旨を添えて人間に相談する。
